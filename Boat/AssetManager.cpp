...
AssetManager::AssetManager() : assetLoader(), objectsToLoad(), models(), textures(), shaders(), objectsMutex(NULL), modelsMutex(NULL), texturesMutex(NULL), shadersMutex(NULL)
{
	...
	assetLoader.soundsMutex = &soundsMutex;
}
...
AssetManager::~AssetManager()
{
	...
	//waitResult = WaitForSingleObject(soundsMutex, INFINITE);
	for (soundMap::iterator it = sounds.begin(); it != sounds.end(); it++) {
		delete it->second;
		it->second = 0;
	}
	sounds.clear();
	//if (! ReleaseMutex(soundsMutex)) printf("Error releasing Textures Mutex"); 
	...
	if (soundsMutex != NULL)	CloseHandle(soundsMutex);
}
...
void AssetManager::loadSound(string path)
{
	DWORD waitResult = WaitForSingleObject(soundsMutex, INFINITE);
	soundMap::iterator it = sounds.find(path);
	if (!ReleaseMutex(soundsMutex)) printf("Error releasing Sounds Mutex");
...
void AssetManager::forceLoadSound(string path)
{
	DWORD waitResult = WaitForSingleObject(soundsMutex, INFINITE);
	soundMap::iterator it = sounds.find(path);
	if (it == sounds.end())
		sounds.insert(std::pair<string, Sound*>(path, Sound::loadSound(path)));
	if (!ReleaseMutex(soundsMutex)) printf("Error releasing Sounds Mutex");
}
...
Sound * AssetManager::getSound(string path)
{
	DWORD waitResult = WaitForSingleObject(soundsMutex, INFINITE);
	soundMap::iterator it = sounds.find(path);
	if (!ReleaseMutex(soundsMutex)) printf("Error releasing Sounds Mutex");
